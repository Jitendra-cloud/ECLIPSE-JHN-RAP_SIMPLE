...
*    ENDLOOP.
  ...
  METHOD validateCoreData.
    READ ENTITIES OF ZJS_I_RAPPartner IN LOCAL MODE
         ENTITY Partner
  ...
  METHOD fillCurrency.
    ...
    LOOP AT lt_partner_data INTO DATA(ls_partner) WHERE PaymentCurrency IS INITIAL.
      MODIFY ENTITIES OF ZJS_I_RAPPartner IN LOCAL MODE
             ENTITY Partner
  ...
  METHOD copyLine.
    DATA lt_creation TYPE TABLE FOR CREATE ZJS_I_RAPPartner.
...
CLASS lsc_ZJS_i_rappartner DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS
...
CLASS lsc_ZJS_i_rappartner IMPLEMENTATION.
