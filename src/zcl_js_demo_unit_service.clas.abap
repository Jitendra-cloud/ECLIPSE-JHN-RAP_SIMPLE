"! @testing ZJS_UI_SIMPLE_PARTNER_O2
CLASS zcl_js_demo_unit_service DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
  ...
  PRIVATE SECTION.
    TYPES:
      BEGIN OF ts_key,
        PartnerNumber TYPE ZJS_C_RAPPartner-PartnerNumber,
      END OF ts_key.
...
CLASS ZCL_JS_DEMO_UNIT_SERVICE IMPLEMENTATION.
  ...
  METHOD create_test_client.
    ro_result = cl_web_odata_client_factory=>create_v2_local_proxy(
        VALUE #( service_id = 'ZJS_UI_SIMPLE_PARTNER_O2' service_version = '0001' )
    ).
  ENDMETHOD.
  ...
  METHOD get_all_partners.
    DATA:
      lt_result TYPE TABLE OF ZJS_C_RAPPartner.
  ...
  METHOD update_unison.
    ...
    lo_request->set_business_data( VALUE ZJS_C_RAPPartner( Street = 'Updated' ) ).
    lo_request->execute( ).
