"! @testing ZJS_I_RAPPartner
CLASS zcl_js_demo_unit_rap DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS ZCL_JS_DEMO_UNIT_RAP IMPLEMENTATION.


  METHOD class_setup.
    DATA:
      lt_partner TYPE STANDARD TABLE OF ZJS_dmo_partner WITH EMPTY KEY.

    go_environment = cl_cds_test_environment=>create(
        i_for_entity = 'ZJS_I_RAPPARTNER'
        i_dependency_list = VALUE #( ( name = 'ZJS_DMO_PARTNER' type ='TABLE' ) )
    ).
  ...
  METHOD clear_empty_streets.
    DATA:
      lt_clear_streets TYPE TABLE FOR ACTION IMPORT ZJS_I_RAPPartner\\Partner~clearAllEmptyStreets.

    INSERT INITIAL LINE INTO TABLE lt_clear_streets.

    MODIFY ENTITIES OF ZJS_I_RAPPartner
    ...
    COMMIT ENTITIES
      RESPONSE OF ZJS_I_RAPPartner
        REPORTED DATA(ls_commit_reported)
        FAILED DATA(ls_commit_failed).

    SELECT FROM ZJS_dmo_partner
      FIELDS partner
      WHERE street = 'EMPTY'
  ...
  METHOD create_new_entry.
    DATA:
      lt_new_partner TYPE TABLE FOR CREATE ZJS_I_RAPPartner.
    ...
    SELECT SINGLE FROM ZJS_dmo_partner
      FIELDS partner, name
  ...
  METHOD fill_empty_streets.
    DATA:
      lt_fill_streets TYPE TABLE FOR ACTION IMPORT ZJS_I_RAPPartner\\Partner~fillEmptyStreets.
